...
def gradio_ask_multiple_images(images, user_message , temperature_input):
    ...
    for image_path in images:
        ...
        cb, cs = gradio_stream_answer_new(chatbot, chat_state, img_list, temperature_input)
        # Regular expression pattern to match the number
        number = re.search(r'/(\d+)_Image', image_path).group(1)
        #print(cb)
        all_text_outputs += f"Image name: {image_path}, folder_num: {number} ,chatbot reply: {cb}\n"
        cb_strip = cb[0][1] 
        csvdata.append((image_path, cb_strip))
    ...
    with open("llm_messages_csv.csv", "w", newline='') as file:
        ...
        writer.writerow(["Image Path","folder", "Chatbot Reply"])  # Write header
        for image_path, cb in csvdata:
            number = re.search(r'/(\d+)_Image', image_path).group(1)
            writer.writerow([image_path, number, cb])
...
with gr.Blocks() as demo:
    with gr.Row():
        ...
        temperature_input = gr.Slider(minimum=0, maximum=1, step=0.01, value=1, label="Temperature")
