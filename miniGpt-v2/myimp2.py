...
# Function to recursively get all file names and paths within a directory @Malik
def get_all_file_paths(directory):
    file_paths = []
    for root, dirs, files in os.walk(directory):
        for file in files:
            if not file.startswith('._'):  # for my mac
                file_path = os.path.join(root, file)
                file_paths.append(file_path)
    return file_paths

# Function to handle the uploaded zip file @Malik
def process_zip_file(file_info):
    ...
    # Get all file paths within the extracted directory
    image_paths = get_all_file_paths(extract_path) 
    # Return the list of image names and the images themselves
    return image_paths
...
# getting text from multiple image processing @malik
def gradio_ask_multiple_images(images, user_message, chatbot, chat_state, gr_img, img_list, upload_flag, replace_flag, temperature):
    if len(user_message) == 0:
        text_box_show = 'Input should not be empty!'
    else:
        text_box_show = ''
    all_text_outputs = ""  # To store all text outputs
    for image_path in images:
        #Lists to save the images and answers 
        #reinatialized everytime start of for loop
        img_list = []
        chatbot = []
        gr_img = image_path 
        print(type(gr_img))
        ...
        # Use gradio_stream_answer and expect it to return full output
        ...
        # Saving the answer from the chatbot for each image
        cb, cs = gradio_stream_answer_new(chatbot, chat_state, img_list, temperature) 
        #print(cb)
        all_text_outputs += f"Image name: {image_path}, chatbot reply: {cb}\n"

    # Write all_text_outputs to a text file
    with open("llm_messages.txt", "w") as text_file:
        text_file.write(all_text_outputs)

    return text_box_show, chatbot, chat_state, img_list, upload_flag, replace_flag, all_text_outputs
...
def gradio_answer(chatbot, chat_state, img_list, temperature):
    ...
    llm_message = chat.answer(conv=chat_state,
                              ...
                              max_length=2000)[0]
    chatbot = llm_message
    all_text_outputs += f"User message: {chatbot}\n"
...
def gradio_stream_answer(chatbot, chat_state, img_list, temperature):
    ...
    all_text_outputs += f"User message: {chatbot} , {len(img_list)}\n"
...
def gradio_stream_answer_new(chatbot, chat_state, img_list, temperature):
    ...
    output = ''
    for new_output in streamer:
        escaped = escape_markdown(new_output)
        output += escaped
    # Update chatbot with complete output after collecting all responses
    chatbot.append([None, output])
    return chatbot, chat_state
...
def gradio_taskselect(idx):
    ...
    instruct_list = [
        '**Hint:** Type in whatever you want test',
...
1. Grounding: CLICK **Send** to generate a grounded image description test.
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=0.5):
            images =gr.Files(label="Upload multiple images", file_count="multiple")
            user_message_input = gr.Textbox(label="Enter your message here", placeholder="Type your message...")
    ...
    # Connecting ZIP file input and user message input to your gradio_ask_multiple_images function
    send.click(
        gradio_ask_multiple_images,
        inputs=[images, user_message_input, chatbot, chat_state, img_list, upload_flag, replace_flag],
        outputs=[user_message_input, chatbot, chat_state, img_list, upload_flag, replace_flag]
    )

    clear.click(
        gradio_reset,
        inputs=[chat_state, img_list],
        outputs=[chatbot, images, user_message_input, chat_state, img_list]
    )
